# Design Document: Does Chain-of-Thought Prefer a Different Persona

## 1. Purpose

This document is the method contract for **Does Chain-of-Thought Prefer a Different Persona**. It specifies what is
...
One-liner: Compare persona markers in final answers versus intermediate reasoning traces.

Hypothesis: Compare persona markers in final answers versus intermediate reasoning traces.
...
|---|---|---|
| Config schema | `cotpersona.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `cotpersona.configs.loader` | Hydra compose API |
| Device | `cotpersona.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `cotpersona.models.registry` | Pinned revisions + hardware fit |
| Hooks | `cotpersona.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `cotpersona.models.generation` | Chat template path recording |
| Data | `cotpersona.data` | Synthetic items, splits, manifests |
| Evaluation | `cotpersona.evaluation` | Metrics, failure harness, protocols |
| Ablation | `cotpersona.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `cotpersona.reporting` | Aggregate → MD/TeX/figures |
| Cache | `cotpersona.cache` | Versioned artifact store |
...
`src/cotpersona/cotpersona/` are preserved across regeneration.
